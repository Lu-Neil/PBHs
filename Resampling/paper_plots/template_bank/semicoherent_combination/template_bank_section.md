...
The metric template-bank construction should not include the Fourier-bin
response.  The sinc-like response of the nearest coherent bin is a
statistic-level normalization or validation correction, not an additional
parameter-space metric factor.  In particular, the constant curvature of the
averaged bin response should not be folded into the bank spacing or template
cell area.

After a bank has been constructed, the loss from reading the nearest coherent
frequency bin can be assessed separately as an average over coherent-bin
offsets.  For a crossing time error `epsilon`, the induced offset in bin units
is
...
detectable at 8 kpc.  It should be used to validate or reweight the recovered
statistic, not to define the metric template-bank covering.
