...
recovery_arr = []
day_space = np.arange(1, 31)

for n_days in day_space:
    print(n_days)
    day = 86400
    duration = n_days*day
    f_signal = 5
...
#     pl.semilogy(nufft_freq_sidereal, nufft_sidereal_power, label='PBH evolution + Doppler + Sidereal')
#     pl.legend()
...
#     pl.savefig("figs/without_source_evo.png")
    ...
    estimator = abs(np.dot(X, np.conj(sidereal.A)) /
                    np.sum(np.abs(sidereal.A)**2))

    recovery = estimator/np.max(nufft_power)
    recovery_arr.append(recovery)

pl.plot(day_space, recovery_arr)
pl.savefig("recovery_vs_duration")
breakpoint()
