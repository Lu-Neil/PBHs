...
#     nufft_amp = plan.execute(np.array(data.astype(complex)))
    nufft_amp = finufft.nufft1d1(np.array(temp_tau), np.array(data.astype(complex)), bin_no)
...
nufft_freq_new, nufft_power_new = nufft(signal_doppler_new, tau, len(signal_doppler))
f0_found_new = nufft_freq_new[np.argmax(nufft_power_new)]*2*np.pi

tau_new = 2*np.pi*integrate.cumulative_trapezoid(f_doppler_new/f0_new, x=t_offset)
nufft_freq_test, nufft_power_test = nufft(signal_doppler_new, tau_new, len(signal_doppler))
...
print("This script demonstrates that the numerically integrated tau still works for signals with the beta and different f0")