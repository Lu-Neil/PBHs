...
# %%
f0_idx = abs(resampler.freqs-(f0*2*np.pi)).argmin()
value = resampler.weights_normalized[f0_idx]
np.angle(value)
...
# %%
np.real(h_est) / np.real(h0 * np.exp(1j*gamma))
...
print(hp_est)
print(h0[0]*sidereal.H_p*np.exp(1j*gamma))
...
print(hc_est)
print(h0[0]*sidereal.H_c*np.exp(1j*gamma))
